...
def oror(i, m=0):
    return int(sum(array(list(binary_repr(bitwise_xor(i, m))), int)) > 0)


def xorxor(i, m=0):
    return int(sum(array(list(binary_repr(bitwise_xor(i, m))), int)) % 2 > 0)
...
def cnf_read(fs, sort_clauses=True, read_xors=False):
    ...
    for f in fs:
        ...
        for l in open(f):
            ...
            if sort_clauses:
                vs = vs[argsort(abs(vs))]
            c.append(vs)
        if all(array([len(r) for r in c]) == len(c[0])):
            c = array(c)
        cs.append(c)
        xs.append(x)
    if len(cs) == 1:
...
def cnf_write(c, filename, xs=None):
    ...
    for i in range(nc):
        f.write(xs[i] + " ".join([str(j) for j in c[i]]) + " 0\n")
    f.close()
