...
metis_import_exception = None
try:
    from metis import part_graph
except (RuntimeError, ImportError) as e:
    metis_import_exception = e
...
def get_cluster_eids(membership, g):
    ...
    ei = []
    for v in vi:
        ei = ei + [g.es.select(_within=v).indices]
    return ei
...
def contract_greedy(
    g, n=0, fedge=find_cheapest_edge, fsize=Graph.maxdegree, combine_attrs=None
):
